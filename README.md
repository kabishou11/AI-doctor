...
- 🤖 **多模型支持**：集成 OpenAI、Anthropic Claude、Google Gemini、魔搭社区等主流 AI 模型
- 💬 **实时讨论**：医生轮流发言，支持打字机效果展示
- 🗳️ **智能评估**：医生互相评估彼此的诊断，自动淘汰不准确的意见
- 📊 **状态监控**：实时显示会诊阶段、轮次、医生状态等信息
- 💾 **会话管理**：自动保存会诊记录，支持多个问诊会话切换
- 🖼️ **ModelScope 图片识别**：图片识别仅使用魔搭社区 / ModelScope，多模态结果自动注入病例与会诊
...
2. 在 **"医生配置"** 标签页中添加医生：
   - 输入医生名称（例如：Dr. GPT-4）
   - 选择供应商（OpenAI规范 / Anthropic规范 / Gemini规范 / 魔搭社区）
   - 填写对应的 API Key
...
2. 如需使用图片识别，请先在 **"全局设置 → 图片识别"** 中启用并配置 ModelScope，再上传病灶图片
...
- **说明**：用于国产大模型与图片识别能力，本项目的图片识别仅支持该供应商
...
- **图片识别**：仅配置 ModelScope 的 API Key、Base URL、多模态模型、提示词和并发数
